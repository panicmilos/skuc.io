...
window._initialize_drawio_diagrams_ = function() {
...
window.addEventListener('resize', debounce(function (event) {
	document.querySelectorAll(".geDiagramContainer")?.forEach(d => {
		processDiagram(d);
		window._drawio_on_rerenders_ && window._drawio_on_rerenders_?.forEach(cb => cb());
	});
}, window._drawio_debounce_timeout_ ?? 500));

window.addEventListener("load", window._initialize_drawio_diagrams_);
...
function processDiagram(diagram, attempt = 10){
	try {
		...
		setTimeout(() => processDiagram(diagram, attempt-1), 500);
	} catch (e) {
		showError(diagram, e);
	}	
}
...
function addDiagram(diagramData, replaceElement, userOptions){
	...
	div.setAttribute('data-diagram-data', diagramData);
	...
	window._drawio_callbacks_ && window._drawio_callbacks_?.forEach(cb => cb());
}
